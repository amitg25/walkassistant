...
extension ViewController {
    ...
    @objc func announcement() {
        ...
        let result = self.boxesView.process()
        if result != "" {
            let speech = "\(result) is ahead of you"
            Speaker.shared.announce(text: speech)
        }
        Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(self.announcement), userInfo: nil, repeats: false)
    }
